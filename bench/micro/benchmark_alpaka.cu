...
static std::unordered_map<std::string, std::string> parse_prm(const std::string &path) {
...
static void check_close(const std::vector<float> &got, const std::vector<float> &ref, 
...
void verify_correctness(Ctx &ctx, int N, const RunCfg &cfg) {
    ...
    gemm::gemm_alpaka_naive(ctx.queue, 
                            alpaka::getPtrNative(bufA_d), 
                            alpaka::getPtrNative(bufB_d), 
                            alpaka::getPtrNative(bufC_d), 
                            shape);
...
static int tb_alpaka_loop(Ctx& ctx, const RunCfg &cfg) {
  ...
  for (int N = cfg.minN; N <= cfg.maxN; N += cfg.step) {
    ...
    std::printf("%s,%s,%d,%.6f,%.6f\n", 
                cfg.solver.c_str(), cfg.precision.c_str(), N, t_ms, gflops);
    std::fflush(stdout);
  }
  return 0;
}
...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (a == "--config") {
      need("--config"); config_file = argv[++i];
    } else if (a == "--check") {
      need("--check"); checkN = std::atoi(argv[++i]);
    } else {
  ...
  auto require_key = [&](const char *k) -> std::string {
    ...
    if (prm.find(key) == prm.end()) { std::fprintf(stderr, "ERROR: missing key '%s' in %s\n", k, config_file.c_str()); std::exit(4); }
    return prm[key];
  };
  ...
  std::cout << "Accelerator: " << alpaka::getAccName<alpaka_bench::Acc>() << "\n";
