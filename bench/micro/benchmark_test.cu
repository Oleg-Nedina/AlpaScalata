 ...
 *
 * Helper utility to normalize configuration keys and values to lowercase
 * for case-insensitive comparison.
 *
 * @param s The input string.
 * @return A new string containing the lowercase version of s.
 */
...
/**
 * @brief Parses a parameter configuration file.
 *
 * Reads a text file containing "key = value" pairs. Lines starting with '#' 
 * are treated as comments. White space around keys and values is trimmed.
 *
 * @param path The file path to the configuration file.
 * @return An unordered_map containing the parsed key-value pairs.
 */
...
/**
 * @brief Checks CUDA error codes and exits on failure.
 *
 * @param e The CUDA error code to check.
 * @param msg The context message to print if an error occurs.
 */
 ...
 *
 * Holds the settings loaded from the .prm file, controlling the
 * range of matrix sizes (N) to test, repetition counts, and random seed.
 */
struct RunCfg {
    int warmup = 5;       // Number of warmup iterations before timing.
    int reps = 20;        // Number of timed repetitions.
    int batch = 1;        // Batch size (usually 1 for single GEMM).

    int minN = 256;       // Minimum matrix dimension N.
    int maxN = 4096;      // Maximum matrix dimension N.
    int step = 256;       // Increment step for N.

    unsigned seed = 123;  // Random seed for matrix initialization.
};

/**
 * @brief Verifies that two vectors are numerically close.
 *
 * Compares a result vector against a reference vector using both absolute
 * and relative tolerances. Exits the program if a mismatch exceeds the tolerance.
 *
 * @param got The vector of calculated values to test.
 * @param ref The vector of reference values.
...
/**
 * @brief Resolves a solver name to a GemmFn function pointer.
 *
 * @param solver The string name of the solver (e.g., "naive").
 * @return Function pointer to the GEMM implementation, or nullptr if unknown.
 */
...
/**
 * @brief Validates a specific solver against the naive implementation.
 *
 * Runs the specified solver and the naive solver on random data for a given
 * matrix size N. Compares the results to ensure correctness.
 *
 * @param N The matrix dimension.
 * @param cfg The run configuration (used for RNG seeding).
 * @param solver_fn The function pointer to the solver being tested.
 */
...
/**
 * @brief Benchmarks the Naive GEMM implementation for a single size N.
 *
 * Allocates memory, initializes data, performs warmup runs, and then measures
 * the execution time of the kernel using CUDA Events.
 *
 * @param N The matrix dimension.
...
/**
 * @brief Runs the benchmark suite for the Naive solver.
 *
 * Iterates through matrix sizes from minN to maxN, running the benchmark
 * for each size and printing the results (Time and GFLOPs) to stdout in CSV format.
 *
 * @param cfg The run configuration.
 * @param solver Solver name string.
 * @param prec Precision string.
 * @return 0 on success.
 */
static int tb_naive_float(const RunCfg &cfg, const std::string &solver,
    ...
    for (int N = cfg.minN; N <= cfg.maxN; N += cfg.step) {
        ...
        std::printf("cuda_%s,%s,%d,%.6f,%.6f\n", solver.c_str(), prec.c_str(), N,
                    t_ms, gflops);
...
/**
 * @brief Benchmarks the Full Options (Optimized) GEMM implementation.
 *
 * Similar to `bench_naive_once_ms`, but invokes `gemm_cuda_full_options`.
 * Includes memory allocation, initialization, warmup, and timing loops.
 *
 * @param N The matrix dimension.
 * @param cfg The run configuration.
 * @return Average execution time in milliseconds.
...
/**
 * @brief Runs the benchmark suite for the Full Options solver.
 *
 * Iterates through matrix sizes, benchmarks the optimized kernel, and prints
 * performance metrics (Time, GFLOPs) to stdout.
 *
 * @param cfg The run configuration.
 * @param solver Solver name string.
 * @param prec Precision string.
 * @return 0 on success.
 */
static int tb_full_options_float(const RunCfg &cfg, const std::string &solver,
                                 const std::string &prec) {
    std::printf("Solver,Precision,Size,Time_ms,GFLOPs\n");
    for (int N = cfg.minN; N <= cfg.maxN; N += cfg.step) {
 ...
 *
 * Parses command-line arguments to load a configuration file and optionally
 * perform a correctness check. Dispatches the appropriate benchmark function
 * based on the 'solver' and 'precision' keys specified in the config file.
 *
 * Usage:
 * ./benchmark_test --config <file.prm> [--check N]
 *
