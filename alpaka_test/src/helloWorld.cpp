/*Copyright 2023 Benjamin Worpitz, Erik Zenker, Bernhard Manfred Gruber, Jan
 * Stephan SPDX-License-Identifier: ISC
 */
...
template <alpaka::concepts::Tag TAccTag> auto example(TAccTag const &) -> int {
  ...
  // synchronizable), whereas the CUDA accelerator can spawn hundreds of
  // synchronizing and non synchronizing threads at the same time.
  //
  // The workdiv is divided in three levels of parallelization:
  // - grid-blocks:      The number of blocks in the grid (parallel, not
  // synchronizable)
  // - block-threads:    The number of threads per block (parallel,
  // synchronizable).
  ...
  // Let alpaka calculate good block and grid sizes given our full problem
  // extent
  auto const workDiv =
      alpaka::getValidWorkDiv(kernelCfg, devAcc, helloWorldKernel);
  ...
  alpaka::exec<Tag>(queue, workDiv, helloWorldKernel
...
TagCpuTbbBlocks,
    ...
    return alpaka::executeForEachAccTag([=](auto const& tag) { return
example(tag); });
}*/

auto main() -> int { return example(alpaka::TagCpuSerial{}); }
