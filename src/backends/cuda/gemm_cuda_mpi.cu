...
int main(int argc, char **argv) {
  ...
  // This simplifies the scatter/gather logic by ensuring every rank processes
  ...
  // Pinned memory is required for optimal bandwidth and is mandatory if using
  // asynchronous copies (cudaMemcpyAsync) overlapping with compute.
  ...
  if (rank == 0) {
    ...
    double gb_req =
        (double)(size_A_full + size_C_full + size_B_pad) * 4.0 / 1e9;
...
#pragma omp parallel for
    ...
    // Zero out padding rows in A if any (crucial to avoid NaN/Inf affecting
    // results)
...
#pragma omp parallel for
  ...
  if (size_B_pad < 2000000000) {
    ...
    if (rank == 0)
      std::cout << "Warning: B too big, local generation." << std::endl;
    // Fallback: Local generation if B is too large for a single MPI message or
    // to save bandwidth.
    for (int r = 0; r < K_real; ++r)
      for (int c = 0; c < N_real; ++c)
        h_B[r * N_pad + c] = 1.0f;
  ...
  MPI_Scatter(h_A_full, M_local_pad * K_pad, MPI_FLOAT, h_A_local,
  ...
  if (rank == 0)
    std::cout << ">>> START GPU (CUDA) <<<" << std::endl;
  ...
  MPI_Gather(h_C_local, M_local_pad * N_pad, MPI_FLOAT, h_C_full,
